  ...
  <div class="chat-area">
    ...
    <div class="model-bar">
      ...
      <div class="mbar-right">
        ...
        <button class="icon-btn" id="ejectBtn" onclick="ejectModel()" title="Eject model from memory">⏏</button>
...
async function ejectModel() {
  if (!S.model) { toast('No model selected', 'err'); return; }
  if (S.provider !== 'ollama') { toast('Eject only works with Ollama', 'err'); return; }
  const btn = document.getElementById('ejectBtn');
  btn.textContent = '⟳'; btn.disabled = true;
  setStatus('busy', 'Ejecting…');
  try {
    // Ollama unloads a model by sending keep_alive: 0
    const r = await fetch('/proxy/ollama/api/generate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: S.model, keep_alive: 0 }),
    });
    if (r.ok) {
      toast('⏏ ' + S.model + ' ejected from memory', 'ok');
      setStatus('idle', 'Model ejected');
      btn.classList.add('active');
    } else {
      const err = await r.json().catch(() => ({}));
      toast('Eject failed: ' + (err.error || r.status), 'err');
      setStatus('online', S.model);
    }
  } catch(e) {
    toast('Eject error: ' + e.message, 'err');
    setStatus('online', S.model);
  }
...
function renderMessageStreaming(msg) {
  ...
  // Build every element directly — direct JS reference to bubble,
  // no getElementById/querySelector, zero collision with other messages.
  ...
  row.dataset.idx = String(S.messages.length - 1);
